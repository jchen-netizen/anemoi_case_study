#!/bin/bash
# Usage:
#   bash run_wrf2verif_wind.sh <base_path> <output.nc> <start_date> <end_date>
#
...
#
# Example:
#   bash run_wrf2verif_wind.sh /Volumes/webdav/Share_Forecasts/WAC00WG-01 \
...
if [ -z "$BASE_PATH" ] || [ -z "$OUTPUT_NC" ] || [ -z "$START" ] || [ -z "$END" ]; then
    echo "Usage: bash run_wrf2verif_wind.sh <base_path> <output.nc> <start_date YYMMDDHH> <end_date YYMMDDHH>"
    exit 1
fi
...
while :; do
    ...
    if [ -e "${FILES[0]}" ]; then
        ...
        python3 "${SCRIPT_DIR}/wrf2verif_wind.py" "${FILES[@]}" "$INIT_TIME" "$OUTPUT_NC"
    else
